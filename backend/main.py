...
_ Do not call get_worldcup if the user prompt asks about countries in general.
...
def get_superhero(name):
    ...
    try:
        ...
        superhero_data = json.loads(response.text)["results"][0]
        ...
        return f"I am sorry but I could not find any information about {name}"
...
def call_gemini(user_prompt: Annotated[str, Body()]):
    ...
    for step in response.steps:
        match step.type:
            case "model_output":
                return "I am sorry but i can only answer questions related to superheros or the fifa world cup!"
